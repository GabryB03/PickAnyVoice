...
class FirefoxMainTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            if (i == 0)
            {
                G.DrawImage(Resources.Tab1, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 26, 26));
            }
            else if (i == 1)
            {
                G.DrawImage(Resources.Tab2, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 26, 26));
            }
        }
